...
def parse_arguments():
    ...
    parser.add_argument('--lutris', action='store_true', help='install wine-ge for lutris')
...
def main():
    ...
    if args.dir:
        print(f"Install directory set to '{install_directory(target=args.dir, lutris=args.lutris)}'")
    ...
    if args.tag or not (args.remove or args.list or args.dir or args.releases):
        get_proton(version=args.tag, yes=args.yes, dl_only=args.download,
                   output=args.output, lutris=args.lutris)
    if args.remove:
    ...
    if args.list:
        _install_directory = install_directory(lutris=args.lutris)
        for item in installed_versions(lutris=args.lutris):
            print(f"{item} - {readable_size(folder_size(_install_directory + item))}")

    if args.releases:
        for tag in fetch_releases(lutris=args.lutris):
            print(tag)
